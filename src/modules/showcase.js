...
/**
 * A lens that reveals the finished design through a wireframe. The mask centre
 * is lerped toward the pointer so the lens trails slightly, matching the weight
 * of the cursor trail elsewhere on the site.
 *
 * Touch devices and reduced-motion get a static split instead, handled in CSS.
 */
export function initSpotlight() {
	...
	const lens = stage.querySelector('[data-spot-lens]');
	const ring = stage.querySelector('[data-spot-ring]');
	if (!lens) return;
	...
	stage.addEventListener('pointerenter', (event) => {
		...
		const rect = stage.getBoundingClientRect();
		target.x = eased.x = event.clientX - rect.left;
		target.y = eased.y = event.clientY - rect.top;
	...
	function frame() {
		...
		lens.style.setProperty('--lens-x', `${eased.x}px`);
		lens.style.setProperty('--lens-y', `${eased.y}px`);
		if (ring) {
			ringX(eased.x);
			ringY(eased.y);
		}

		const settled = Math.hypot(target.x - eased.x, target.y - eased.y) < 0.3;
		if (!inside && settled) {
			running = false;
			return;
		}
